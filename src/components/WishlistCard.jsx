  ...
  return (
    <div className="mt-15">
      <div className="relative h-[250px] flex items-center justify-center bg-[#F5F5F5]">
        <div className="absolute top-3 left-3 bg-[#DB4444] text-white font-normal text-xs leading-[18px] font-poppins py-1 px-3 rounded-md">{item.new ? <div>NEW</div> : <div>-35%</div>}</div>
      ...
      <div>
        <p className="mt-4 font-medium text-base leading-6 font-poppins">{item.title}</p>
        <div className="flex items-center gap-2">
            <p className="text-[#DB4444] font-medium text-base leading-6 font-poppins">${item.price}</p>
            {
                item.prevPrice && (
                    <s className="text-[#7F7F7F] font-medium text-base leading-6 font-poppins">${item.prevPrice}</s>
                )
            }
        </div>
        {
